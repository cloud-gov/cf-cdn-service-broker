...
// Create provides a mock function with given fields: instanceId, domain, origin, path, insecureOrigin
func (_m *RouteManagerIface) Create(instanceId string, domain string, origin string, path string, insecureOrigin bool) (models.Route, error) {
	ret := _m.Called(instanceId, domain, origin, path, insecureOrigin)

	var r0 models.Route
	if rf, ok := ret.Get(0).(func(string, string, string, string, bool) models.Route); ok {
	...
	if rf, ok := ret.Get(1).(func(string, string, string, string, bool) error); ok {
		r1 = rf(instanceId, domain, origin, path, insecureOrigin)
	} else {
		r1 = ret.Error(1)
	}
