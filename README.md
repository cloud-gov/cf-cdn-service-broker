...
A [Cloud Foundry](https://www.cloudfoundry.org/) [service broker](https://docs.cloudfoundry.org/services/) that uses [AWS CloudFront](https://aws.amazon.com/cloudfront/) to proxy traffic from a domain that the user controls (the domain) to an existing Cloud Foundry application or external URL. Traffic is encrypted using an SSL certificate generated by [ACM](https://aws.amazon.com/certificate-manager/).

![A sketchnote depicting the CDN Broker's use-case and related components](docs/cdn-broker-intro-sketchnote.jpg "By @46bit")
